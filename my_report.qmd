...
# Directories
raw_dir = 'D:/ParisC/Projects/BigData-HWI/DATA/Raw'
cleaned_dir = 'D:/ParisC/Projects/BigData-HWI/DATA/Cleaned'
...
input_dir = Path('D:/ParisC/Projects/BigData-HWI/DATA/Cleaned')
output_dir = Path('D:/ParisC/Projects/BigData-HWI/NLP/annotated')
...
# Directories
cleaned_dir = Path("D:/ParisC/Projects/BigData-HWI/DATA/Cleaned")
output_dir = Path("D:/ParisC/Projects/BigData-HWI/NLP/Stylo_Analysis_results")
output_dir.mkdir(parents=True, exist_ok=True)
...
with open("D:/ParisC/Projects/BigData-HWI/NLP/Stylo_Analysis_results/analysis_results.json", encoding='utf-8') as f:
    data = json.load(f)
...
After conducting the stylometric analysis, we can visualize the distribution of the average sentence length across the texts. 
This graphic illustrates how sentence length varies within the analyzed works. A **peak** in the distribution indicates a common sentence length used throughout the text, while a **wider** spread suggests more variability. By examining this distribution, we can gain insights into the author's writing style, such as whether they tend to favor shorter, punchy sentences or longer, more complex structures. 
...
ttr_df = pd.DataFrame({
    "Text": list(data.keys()),
    "TTR": [v["TTR"] for v in data.values()]
})
...
alt.Chart(ttr_df).mark_bar().encode(
    ...
    y=alt.Y('TTR:Q'),
    tooltip=['Text', 'TTR']
).properties(title="Type-Token Ratio Distribution", width=600)
```


## Global Word Cloud

Here is the global word cloud generated from all 92 Balzac novels:

![Word Cloud](D:/ParisC/Projects/BigData-HWI/NLP/Visualisations/wordcloud_spacy_all.png)
...
# Paths
cleaned_dir = Path("D:/ParisC/Projects/BigData-HWI/DATA/Cleaned")
output_image_path = Path("D:/ParisC/Projects/BigData-HWI/NLP/Visualisations/wordcloud_spacy_all.png")
